...
2. Open the authorized keys\_file: `vim /home/<username>/.ssh/authorized_keys`
4. Populate it with the **public** RSA-keys you wish to grand access to the container (change with your keys):
    ```shell
...
1. Open the lxc-auto file: `vim /etc/config/lxc-auto`
2. Adapt the file as follows:
    ```
    config container
